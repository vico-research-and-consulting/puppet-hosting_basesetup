...
case $TERM in
    xterm*|linux|screen*)
       ...
       export PS1='$(EXC=$?;if [ $EXC != 0 ] ;then echo \[\e[31m\]ERR "$EXC : " ; fi)\[\e]0;\w\a\]\[\e[32m\]\u@\H(\D{%Y-%m-%d} \t) \[\e[93m\][PUPPET]\[\e[0m\]\[\e[31m\]$(KXC=$(kubectl config current-context 2>/dev/null); if [ -n "$KXC" ] && [ "$KXC" != "Please ask your administrator." ];then echo " $KXC "; else echo " " ; fi)\[\e[33m\]\w\[\e[0m\]$(GT=$(git branch 2>/dev/null|grep "*"); if [ -n "$GT" ];then echo -e -n "\E[3m [${GT#* }] \E[23m "; fi) \n\$ \[\e]2;\H \w\a\]'
    ;;
esac
